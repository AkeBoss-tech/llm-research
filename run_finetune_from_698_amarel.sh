#!/bin/bash
#SBATCH --job-name=finetune_from_698
#SBATCH --output=finetune_from_698_%j.out
#SBATCH --error=finetune_from_698_%j.err
...
# ==========================================
# Amarel Cluster Script: Continue Finetuning from Step 698
# ==========================================
# This script continues finetuning from the checkpoint at step 698.
...
CHECKPOINT_DIR="$HOME/llm-research/output/default_finetune"
STEP=698
...
if [ -d "/scratch/$USER" ] && [ -w "/scratch/$USER" ]; then
    OUTPUT_DIR="/scratch/$USER/gsm8k_finetune_from_698/job_${SLURM_JOB_ID}"
    echo "Using /scratch for output (auto-purged after 90 days)"
else
    OUTPUT_DIR="$HOME/gsm8k_finetune_from_698/job_${SLURM_JOB_ID}"
...
# Set wandb run name
WANDB_RUN="${WANDB_RUN:-gsm8k-from-698-${SLURM_JOB_ID}}"
echo "Wandb run name: $WANDB_RUN"

# Run finetuning
echo "Starting GSM8K finetuning from checkpoint step $STEP..."
...
try:
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("GSM8K Finetuning Summary (Continued from Step 698)\n")
        f.write("=" * 80 + "\n")
